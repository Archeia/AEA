...
def targets_for_opponents_advai
  if item.for_random?
    ...
    if @target_index < 0
      # Start with all alive opponents
      pool = opponents_unit.alive_members.dup

      # Keep only HP ≥ t_hpgt
      if @t_conditions[:t_hpgt]
        thresh = @t_conditions[:t_hpgt] / 100.0
        pool.reject! { |a| a.hp_rate < thresh }
      end

      # Keep only HP ≤ t_hplt
      if @t_conditions[:t_hplt]
        thresh = @t_conditions[:t_hplt] / 100.0
        pool.reject! { |a| a.hp_rate > thresh }
      end

      # Must have these states
      if @t_conditions[:t_states]
        @t_conditions[:t_states].each do |state_id|
          pool.reject! { |a| !a.state?(state_id) }
        end
      end

      # Must NOT have these states
      if @t_conditions[:t_states_not]
        @t_conditions[:t_states_not].each do |state_id|
          pool.reject! { |a| a.state?(state_id) }
        end
      end

      # Fallback or pick from pool
      if pool.empty? && !@t_conditions[:t_absoluteneed]
        # no valid target → fall back to any random opponent
        [opponents_unit.random_target] * num
      elsif pool.empty?
        # absolute‐need, but no one qualifies → skip skill
        nil
      else
        # pick one at random from the qualified pool
        [pool.sample] * num
      end

    else
      # a fixed index was set, use that
      [opponents_unit.smooth_target(@target_index)] * num
    end

  else
    # multi‐target skills hit everyone alive
...
def targets_for_friends_advai
    if item.for_user?
      ...
      if item.for_one?
        ...
        # … your t_states / t_states_not filters …
        ...
        if pool.empty? && !@t_conditions[:t_absoluteneed]
          ...
          lowest = pool.min_by { |a| a.hp_rate }
          [ lowest ] * num
        end
